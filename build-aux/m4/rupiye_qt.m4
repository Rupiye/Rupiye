...
dnl Output: If qt version is auto, set rupiye_enable_qt to false. Else, exit.
AC_DEFUN([Rupiye_QT_FAIL],[
  if test "x$rupiye_qt_want_version" = xauto && test "x$rupiye_qt_force" != xyes; then
    if test "x$rupiye_enable_qt" != xno; then
      AC_MSG_WARN([$1; rupiye-qt frontend will not be built])
    fi
    rupiye_enable_qt=no
    rupiye_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([Rupiye_QT_CHECK],[
  if test "x$rupiye_enable_qt" != xno && test "x$rupiye_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl Rupiye_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([Rupiye_QT_PATH_PROGS],[
  Rupiye_QT_CHECK([
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      Rupiye_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other Rupiye_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([Rupiye_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build Rupiye-qt GUI (default=auto)])],
    [
     Rupiye_qt_want_version=$withval
     if test "x$Rupiye_qt_want_version" = xyes; then
       Rupiye_qt_force=yes
       Rupiye_qt_want_version=auto
     fi
    ],
    [Rupiye_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); Rupiye_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   Rupiye_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _Rupiye_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: Rupiye_enable_qt, Rupiye_enable_qt_dbus, Rupiye_enable_qt_test
AC_DEFUN([Rupiye_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  Rupiye_QT_CHECK([_Rupiye_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _Rupiye_QT_CHECK_STATIC_PLUGIN does
  ...
  _Rupiye_QT_IS_STATIC
  if test "x$Rupiye_cv_static_qt" = xyes; then
    _Rupiye_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _Rupiye_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _Rupiye_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      _Rupiye_QT_CHECK_STATIC_PLUGIN([QWindowsVistaStylePlugin], [-lqwindowsvistastyle])
      ...
      _Rupiye_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _Rupiye_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _Rupiye_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  Rupiye_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  Rupiye_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  Rupiye_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  Rupiye_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  Rupiye_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  Rupiye_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  Rupiye_QT_CHECK([
    Rupiye_enable_qt=yes
    Rupiye_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      Rupiye_enable_qt_test=no
    fi
    Rupiye_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      Rupiye_enable_qt_dbus=yes
    fi
  ...
  if test x$Rupiye_enable_qt = xyes; then
    AC_MSG_RESULT([$Rupiye_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$Rupiye_enable_qt])
  fi
...
dnl
dnl _Rupiye_QT_IS_STATIC
dnl ---------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
dnl Output: Rupiye_cv_static_qt=yes|no
AC_DEFUN([_Rupiye_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, Rupiye_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [Rupiye_cv_static_qt=yes],
      [Rupiye_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _Rupiye_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_Rupiye_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); Rupiye_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _Rupiye_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_Rupiye_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _Rupiye_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_Rupiye_QT_FIND_LIBS],[
  Rupiye_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [Rupiye_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  Rupiye_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [Rupiye_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  Rupiye_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [Rupiye_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  Rupiye_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [Rupiye_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
