#!/usr/bin/env bash
#
# Copyright (c) 2019-2020 The Rupiye Core developers
...
TEST_RUPIYE_BINARY="src/test/test_rupiye"
...
if [[ ! -e ${TEST_RUPIYE_BINARY} ]]; then
    echo "Error: Executable ${TEST_RUPIYE_BINARY} not found. Run \"./configure --enable-lcov\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_RUPIYE_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
