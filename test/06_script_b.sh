...
if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
  DOCKER_EXEC ${TEST_RUNNER_ENV} DIR_UNIT_TEST_DATA=${DIR_UNIT_TEST_DATA} LD_LIBRARY_PATH=$DEPENDS_DIR/$HOST/lib "${BASE_BUILD_DIR}/-*/src/test/test_*" --catch_system_errors=no -l test_suite
fi
...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  DOCKER_EXEC LD_LIBRARY_PATH=$DEPENDS_DIR/$HOST/lib test/fuzz/test_runner ${FUZZ_TESTS_CONFIG} $MAKEJOBS -l DEBUG ${DIR_FUZZ_IN}
fi
